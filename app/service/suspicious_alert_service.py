...
class SuspiciousAlertService:
    ...
    async def maybe_record_login_alert(
        ...
        *,
        ...
        shared_ip_users = await cls._distinct_other_users_by_ip(session, ip_address, exclude_user_id=user_id)
        if len(shared_ip_users) >= settings.suspicious_shared_ip_user_threshold:
        ...
        shared_uuid_users: list[int] = []
        if web_uuid:
            shared_uuid_users = await cls._distinct_other_users_by_web_uuid(session, web_uuid, exclude_user_id=user_id)
            if shared_uuid_users:
                reasons.append(f"web UUID already seen on {len(shared_uuid_users)} other account(s)")
                severity = "critical"
        ...
        return await cls._create_alert(
            ...
            body=f"{username} (#{user_id}) logged in with suspicious signals: " + "; ".join(reasons),
            user_id=user_id,
            payload={
                "username": username,
                "user_id": user_id,
                "country_code": country_code,
                "join_date": join_date.isoformat() if join_date is not None else None,
                "ip_address": ip_address,
                "web_uuid": web_uuid,
    ...
    async def maybe_record_suspicious_score_alert(
        ...
        *,
        ...
        last_client_hash = await redis.get(f"metadata:user:last_client_hash:{user_id}")
        ...
        fingerprint = cls._fingerprint("score", score.id, user_id, score.beatmap_id, f"{pp_value:.2f}")
        ...
        return await cls._create_alert(
            ...
            severity=severity,
            fingerprint=fingerprint,
            title=f"Suspicious play: {username} {pp_value:.2f}pp",
            body=(
                f"{username} submitted {pp_value:.2f}pp on {beatmap_name} "
